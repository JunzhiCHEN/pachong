...
def get_intro(company_id):
    ...
    url = 'https://rong.36kr.com/n/api/company/%d?asEncryptedTs=0.26437019827021124&asTs=1528670405474' % company_id
...
def get_company_id_list(table_name):
    db = _mysql_config['pachong']['master']
    sql = 'select company_id from %s limit 577, 200  ' % table_name
    result = db.query(sql)
    company_id_list = []
    for one in result:
